				       ...
				       unsigned long pm_event, void *v)
{
	...
	case PM_SUSPEND_PREPARE:
		...
		if (pm_qos_request_active(&max_cpu_qos_blank))
			pm_qos_update_request(&max_cpu_qos_blank, freq_max[CA7]);
		else
			pm_qos_add_request(&max_cpu_qos_blank, PM_QOS_CPU_FREQ_MAX, freq_max[CA7]);
...
static int __init exynos_cpufreq_init(void)
{
	...
	if (pmic_is_jig_attached) {
		...
		pm_qos_update_request_timeout(&boot_max_cpu_qos, 1000000, 360000 * 1000);
		pm_qos_add_request(&boot_min_cpu_qos, PM_QOS_CPU_FREQ_MIN, 0);
		pm_qos_update_request_timeout(&boot_min_cpu_qos, 1000000, 360000 * 1000);
	} else {
		pm_qos_add_request(&boot_max_cpu_qos, PM_QOS_CPU_FREQ_MAX, freq_max[CA15]);
		pm_qos_update_request_timeout(&boot_max_cpu_qos, 1500000, 40000 * 1000);
		pm_qos_add_request(&boot_min_cpu_qos, PM_QOS_CPU_FREQ_MIN, 0);
		pm_qos_update_request_timeout(&boot_min_cpu_qos, 1500000, 40000 * 1000);
	}
#else
	pm_qos_add_request(&boot_max_cpu_qos, PM_QOS_CPU_FREQ_MAX, freq_max[CA15]);
