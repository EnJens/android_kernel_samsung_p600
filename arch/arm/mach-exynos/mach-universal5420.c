...
#include "board-universal5420-wlan.h"
...
static struct s3c2410_uartcfg universal5420_uartcfgs[] __initdata = {
	[0] = {
		.hwport		= 0,
		.flags		= 0,
...
/* ADC */
static struct s3c_adc_platdata universal5420_adc_data __initdata = {
	.phy_init	= s3c_adc_phy_init,
	.phy_exit	= s3c_adc_phy_exit,
};

/* WDT */
static struct s3c_watchdog_platdata universal5420_watchdog_platform_data = {
	exynos_pmu_wdt_control,
	PMU_WDT_RESET_TYPE1,
};

static struct platform_device *universal5420_devices[] __initdata = {
	&ramconsole_device,
...
static void __init universal5420_map_io(void)
{
	...
	s3c24xx_init_uarts(universal5420_uartcfgs, ARRAY_SIZE(universal5420_uartcfgs));
...
static void __init universal5420_init_early(void)
{
	persistent_ram_early_init(&universal5420_pr);
#ifdef CONFIG_SEC_DEBUG
	sec_debug_magic_init();
#endif
}

static void __init universal5420_machine_init(void)
{
	s3c_adc_set_platdata(&universal5420_adc_data);
	s3c_watchdog_set_platdata(&universal5420_watchdog_platform_data);

#ifdef CONFIG_EXYNOS_FIQ_DEBUGGER
	exynos_serial_debug_init(2, 0);
#endif
	exynos5_universal5420_pmic_init();
	exynos5_universal5420_power_init();
	...
	brcm_wlan_init();
	if (lpcharge == 0) {
		exynos5_universal5420_sensor_init();
#ifdef CONFIG_NFC_DEVICES
		exynos5_universal5420_nfc_init();
#endif
	}
	else {
	...
	exynos5_universal5420_led_init();
#endif
#ifdef CONFIG_W1
	exynos5_universal5420_cover_id_init();
#endif
	exynos5_universal5420_fpga_init();
	exynos5_universal5420_thermistor_init();
	ramconsole_pdata.bootinfo = exynos_get_resetreason();
	platform_add_devices(universal5420_devices, ARRAY_SIZE(universal5420_devices));

	/*
	 * Please note that exynos5_universal5420_gpio_init() should be called
